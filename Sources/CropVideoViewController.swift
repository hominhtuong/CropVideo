...
public class CropVideoViewController: UIViewController {
    ...
    private var userTapMargin: CGFloat {
        return configs.sizeConfigs.userTapMargin
    }
    
    private var dotSize: CGFloat {
        return configs.sizeConfigs.dotSize
    }
    
    private var cropAreaViewMinWidth: CGFloat {
        return configs.sizeConfigs.cropAreaViewMinWidth ?? maxWidth * 0.2
    }
    private var cropAreaViewMinHeight: CGFloat {
        return configs.sizeConfigs.cropAreaViewMinHeight ?? maxWidth * 0.2
    }
    
    private var cropViewDefaultWidth: CGFloat {
        return configs.sizeConfigs.cropViewDefaultWidth ?? maxWidth * 0.5
    }
    private var cropViewDefaultHeight: CGFloat {
        return configs.sizeConfigs.cropViewDefaultHeight ?? maxWidth * 0.5
    }
...
extension CropVideoViewController {
    ...
    private func setupView() {
        ...
        cropAreaView >>> cropContainerView >>> {
            $0.frame = CGRect(
                ...
                width: cropViewDefaultWidth,
                height: cropViewDefaultHeight)
            $0.backgroundColor = .clear
        ...
        topDotView >>> cropContainerView >>> {
            $0.snp.makeConstraints {
                ...
                $0.width.height.equalTo(dotSize)
            }
    ...
    private func updateUI() {
        ...
        // Update size
        let size = configs.sizeConfigs
        topDotView.snp.updateConstraints {
            $0.width.height.equalTo(size.dotSize)
        }
        topDotView.layer.cornerRadius = size.dotSize / 2
        
        rightDotView.snp.updateConstraints {
            $0.width.height.equalTo(size.dotSize)
        }
        rightDotView.layer.cornerRadius = size.dotSize / 2
        
        bottomDotView.snp.updateConstraints {
            $0.width.height.equalTo(size.dotSize)
        }
        bottomDotView.layer.cornerRadius = size.dotSize / 2
        
        leftDotView.snp.updateConstraints {
            $0.width.height.equalTo(size.dotSize)
        }
        leftDotView.layer.cornerRadius = size.dotSize / 2
        
        cropAreaView.frame = CGRect(
            x: ((maxWidth - cropViewDefaultWidth) / 2),
            y: (maxHeight - cropViewDefaultHeight) / 2,
            width: cropViewDefaultWidth,
            height: cropViewDefaultHeight)
